window.MacroRobotData = {
  "updatedAt": "2026/06/17 07:46",
  "status": "運作中",
  "summary": [
    {
      "label": "本週事件",
      "value": 33,
      "unit": "個",
      "icon": "calendar"
    },
    {
      "label": "即將公布",
      "value": 101,
      "unit": "個",
      "icon": "file"
    },
    {
      "label": "待補實際值",
      "value": 3,
      "unit": "個",
      "icon": "target"
    },
    {
      "label": "高影響事件",
      "value": 24,
      "unit": "個",
      "icon": "alert",
      "accent": "orange"
    },
    {
      "label": "下一個事件",
      "value": "通膨率",
      "unit": "倒數 10 小時",
      "icon": "filter"
    }
  ],
  "events": [
    {
      "id": "macro-s-p-global-composite-pmi-final-may-20260603",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (May)",
      "sourcePublishTime": "2026/06/03 15:50 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/03 15:50",
      "previous": "47.6",
      "forecast": "43.5",
      "actual": "44.9",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 15:50",
      "tags": [
        "法國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/03 15:50 公布，市場關注前值 47.6、預期 43.5。",
      "event": "法國 綜合 PMI，前值 47.6、預期 43.5、實際 44.9。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (May)",
      "sourcePublishTime": "2026/06/03 15:50 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/03 15:50",
      "previous": "46.5",
      "forecast": "42.9",
      "actual": "44.3",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 15:50",
      "tags": [
        "法國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 15:50 公布，市場關注前值 46.5、預期 42.9。",
      "event": "法國 服務業 PMI，前值 46.5、預期 42.9、實際 44.3。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-may-20260603",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (May)",
      "sourcePublishTime": "2026/06/03 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/03 15:55",
      "previous": "48.4",
      "forecast": "48.6",
      "actual": "48.8",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 15:55",
      "tags": [
        "德國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/03 15:55 公布，市場關注前值 48.4、預期 48.6。",
      "event": "德國 綜合 PMI，前值 48.4、預期 48.6、實際 48.8。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (May)",
      "sourcePublishTime": "2026/06/03 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/03 15:55",
      "previous": "46.9",
      "forecast": "47.8",
      "actual": "48.1",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 15:55",
      "tags": [
        "德國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 15:55 公布，市場關注前值 46.9、預期 47.8。",
      "event": "德國 服務業 PMI，前值 46.9、預期 47.8、實際 48.1。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-may-20260603",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (May)",
      "sourcePublishTime": "2026/06/03 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/03 16:00",
      "previous": "—",
      "forecast": "47.5",
      "actual": null,
      "status": "等待公布",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 16:00",
      "tags": [
        "歐元區",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/03 16:00 公布，市場關注前值 —、預期 47.5。",
      "event": "歐元區 綜合 PMI，前值 —、預期 47.5、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (May)",
      "sourcePublishTime": "2026/06/03 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/03 16:00",
      "previous": "—",
      "forecast": "46.4",
      "actual": null,
      "status": "等待公布",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 16:00",
      "tags": [
        "歐元區",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 16:00 公布，市場關注前值 —、預期 46.4。",
      "event": "歐元區 服務業 PMI，前值 —、預期 46.4、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-may-20260603",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (May)",
      "sourcePublishTime": "2026/06/03 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/03 16:30",
      "previous": "52.6",
      "forecast": "48.5",
      "actual": "49.7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 16:30",
      "tags": [
        "英國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/03 16:30 公布，市場關注前值 52.6、預期 48.5。",
      "event": "英國 綜合 PMI，前值 52.6、預期 48.5、實際 49.7。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (May)",
      "sourcePublishTime": "2026/06/03 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/03 16:30",
      "previous": "52.7",
      "forecast": "47.9",
      "actual": "49.3",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 16:30",
      "tags": [
        "英國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 16:30 公布，市場關注前值 52.7、預期 47.9。",
      "event": "英國 服務業 PMI，前值 52.7、預期 47.9、實際 49.3。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ppi-mom-apr-20260603",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI MoM (Apr)",
      "sourcePublishTime": "2026/06/03 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/03 17:00",
      "previous": "3.4",
      "forecast": "0.4",
      "actual": "0.6",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 17:00",
      "tags": [
        "歐元區",
        "生產者物價指數",
        "偏多"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/06/03 17:00 公布，市場關注前值 3.4、預期 0.4。",
      "event": "歐元區 生產者物價指數 PPI，前值 3.4、預期 0.4、實際 0.6。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-may-20260603",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change (May)",
      "sourcePublishTime": "2026/06/03 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 20:15",
      "previous": "105",
      "forecast": "117",
      "actual": "122",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 20:15",
      "tags": [
        "美國",
        "ADP",
        "偏多"
      ],
      "summary": "ADP 就業人數 將於 2026/06/03 20:15 公布，市場關注前值 105、預期 117。",
      "event": "美國 ADP 就業人數，前值 105、預期 117、實際 122。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-may-20260603",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (May)",
      "sourcePublishTime": "2026/06/03 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 21:45",
      "previous": "51.7",
      "forecast": "51.7",
      "actual": "51.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 21:45",
      "tags": [
        "美國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/03 21:45 公布，市場關注前值 51.7、預期 51.7。",
      "event": "美國 綜合 PMI，前值 51.7、預期 51.7、實際 51.5。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (May)",
      "sourcePublishTime": "2026/06/03 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 21:45",
      "previous": "50.9",
      "forecast": "50.9",
      "actual": "50.7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/03 21:45",
      "tags": [
        "美國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 21:45 公布，市場關注前值 50.9、預期 50.9。",
      "event": "美國 服務業 PMI，前值 50.9、預期 50.9、實際 50.7。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ism-services-new-orders-may-20260603",
      "type": "macro",
      "title": "ISM 製造業新訂單",
      "eventName": "ISM 製造業新訂單",
      "originalEventName": "ISM Services New Orders (May)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "53.5",
      "forecast": "52.8",
      "actual": "57.3",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業新訂單 將於 2026/06/03 22:00 公布，市場關注前值 53.5、預期 —。",
      "event": "美國 ISM 製造業新訂單，前值 53.5、預期 —、實際 57.3。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ism-services-employment-may-20260603",
      "type": "macro",
      "title": "ISM 製造業就業",
      "eventName": "ISM 製造業就業",
      "originalEventName": "ISM Services Employment (May)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "48",
      "forecast": "48.1",
      "actual": "47.9",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業就業 將於 2026/06/03 22:00 公布，市場關注前值 48、預期 —。",
      "event": "美國 ISM 製造業就業，前值 48、預期 —、實際 47.9。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ism-services-pmi-may-20260603",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "ISM Services PMI (May)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "53.6",
      "forecast": "53.8",
      "actual": "54.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "服務業",
        "偏多"
      ],
      "summary": "服務業 PMI 將於 2026/06/03 22:00 公布，市場關注前值 53.6、預期 53.8。",
      "event": "美國 服務業 PMI，前值 53.6、預期 53.8、實際 54.5。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ism-services-business-activity-may-20260603",
      "type": "macro",
      "title": "ISM 服務業指數",
      "eventName": "ISM 服務業指數",
      "originalEventName": "ISM Services Business Activity (May)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "55.9",
      "forecast": "—",
      "actual": "57.7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 服務業指數 將於 2026/06/03 22:00 公布，市場關注前值 55.9、預期 —。",
      "event": "美國 ISM 服務業指數，前值 55.9、預期 —、實際 57.7。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-prices-may-20260603",
      "type": "macro",
      "title": "ISM 製造業物價",
      "eventName": "ISM 製造業物價",
      "originalEventName": "ISM Services Prices (May)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "70.7",
      "forecast": "71.3",
      "actual": "71.3",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業物價 將於 2026/06/03 22:00 公布，市場關注前值 70.7、預期 —。",
      "event": "美國 ISM 製造業物價，前值 70.7、預期 —、實際 71.3。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ism-pmi-5-20260603",
      "type": "macro",
      "title": "ISM 製造業指數",
      "eventName": "ISM 製造業指數",
      "originalEventName": "ISM非製造業PMI (5月)",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "53.6",
      "forecast": "53.7",
      "actual": "54.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM",
        "偏多"
      ],
      "summary": "ISM 製造業指數 將於 2026/06/03 22:00 公布，市場關注前值 53.6、預期 53.7。",
      "event": "美國 ISM 製造業指數，前值 53.6、預期 53.7、實際 54.5。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/548474",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-20260603",
      "type": "macro",
      "title": "ISM服務業經濟活動",
      "eventName": "ISM服務業經濟活動",
      "originalEventName": "ISM服務業經濟活動",
      "sourcePublishTime": "2026/06/03 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/03 22:00",
      "previous": "55.9",
      "forecast": "55.0",
      "actual": "57.7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/06/03 22:00",
      "tags": [
        "美國",
        "ISM服務業經濟活動",
        "偏多"
      ],
      "summary": "ISM服務業經濟活動 將於 2026/06/03 22:00 公布，市場關注前值 55.9、預期 55.0。",
      "event": "美國 ISM服務業經濟活動，前值 55.9、預期 55.0、實際 57.7。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "鉅亨網全球經濟指標",
      "sourceUrl": "https://www.cnyes.com/economy/indicator",
      "sourceList": [
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-pmi-20260604",
      "type": "macro",
      "title": "PMI 採購經理人指數",
      "eventName": "PMI 採購經理人指數",
      "originalEventName": "S&P Global營建業PMI",
      "sourcePublishTime": "2026/06/04 15:30 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/04 15:30",
      "previous": "38.1",
      "forecast": "38.0",
      "actual": "39.6",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/04 15:30",
      "tags": [
        "法國",
        "PMI",
        "偏多"
      ],
      "summary": "PMI 採購經理人指數 將於 2026/06/04 15:30 公布，市場關注前值 38.1、預期 38.0。",
      "event": "法國 PMI 採購經理人指數，前值 38.1、預期 38.0、實際 39.6。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "鉅亨網全球經濟指標",
      "sourceUrl": "https://www.cnyes.com/economy/indicator",
      "sourceList": [
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-pmi-20260604",
      "type": "macro",
      "title": "PMI 採購經理人指數",
      "eventName": "PMI 採購經理人指數",
      "originalEventName": "S&P Global營建業PMI",
      "sourcePublishTime": "2026/06/04 15:30 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/04 15:30",
      "previous": "42.1",
      "forecast": "42.0",
      "actual": "42.4",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/04 15:30",
      "tags": [
        "德國",
        "PMI",
        "偏多"
      ],
      "summary": "PMI 採購經理人指數 將於 2026/06/04 15:30 公布，市場關注前值 42.1、預期 42.0。",
      "event": "德國 PMI 採購經理人指數，前值 42.1、預期 42.0、實際 42.4。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "鉅亨網全球經濟指標",
      "sourceUrl": "https://www.cnyes.com/economy/indicator",
      "sourceList": [
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-pmi-20260604",
      "type": "macro",
      "title": "PMI 採購經理人指數",
      "eventName": "PMI 採購經理人指數",
      "originalEventName": "S&P Global營建業PMI",
      "sourcePublishTime": "2026/06/04 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/04 16:30",
      "previous": "39.7",
      "forecast": "40.3",
      "actual": "38.2",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/04 16:30",
      "tags": [
        "英國",
        "PMI",
        "偏空"
      ],
      "summary": "PMI 採購經理人指數 將於 2026/06/04 16:30 公布，市場關注前值 39.7、預期 40.3。",
      "event": "英國 PMI 採購經理人指數，前值 39.7、預期 40.3、實際 38.2。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "鉅亨網全球經濟指標",
      "sourceUrl": "https://www.cnyes.com/economy/indicator",
      "sourceList": [
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-initial-jobless-claims-may-30-20260604",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Initial Jobless Claims (May/30)",
      "sourcePublishTime": "2026/06/04 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/04 20:30",
      "previous": "212",
      "forecast": "213",
      "actual": "225",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/04 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "偏多"
      ],
      "summary": "初領失業救濟金人數 將於 2026/06/04 20:30 公布，市場關注前值 212、預期 213。",
      "event": "美國 初領失業救濟金人數，前值 212、預期 213、實際 225。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.dol.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-gdp-growth-rate-yoy-3rd-est-q1-20260605",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "GDP Growth Rate YoY 3rd Est (Q1)",
      "sourcePublishTime": "2026/06/05 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/05 17:00",
      "previous": "1.2",
      "forecast": "0.8",
      "actual": "0.3",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "高",
      "timestamp": "2026/06/05 17:00",
      "tags": [
        "歐元區",
        "GDP",
        "偏空"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/05 17:00 公布，市場關注前值 1.2、預期 0.8。",
      "event": "歐元區 GDP 經濟成長率，前值 1.2、預期 0.8、實際 0.3。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-employment-change-yoy-final-q1-20260605",
      "type": "macro",
      "title": "就業人數變化",
      "eventName": "就業人數變化",
      "originalEventName": "Employment Change YoY Final (Q1)",
      "sourcePublishTime": "2026/06/05 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/05 17:00",
      "previous": "0.7",
      "forecast": "0.5",
      "actual": "0.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/05 17:00",
      "tags": [
        "歐元區",
        "就業人數變化",
        "中性"
      ],
      "summary": "就業人數變化 將於 2026/06/05 17:00 公布，市場關注前值 0.7、預期 0.5。",
      "event": "歐元區 就業人數變化，前值 0.7、預期 0.5、實際 0.5。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-u-6-unemployment-rate-may-20260605",
      "type": "macro",
      "title": "失業率",
      "eventName": "失業率",
      "originalEventName": "U-6 Unemployment Rate (May)",
      "sourcePublishTime": "2026/06/05 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/05 20:30",
      "previous": "8.2",
      "forecast": "4.3",
      "actual": "8.1",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/05 20:30",
      "tags": [
        "美國",
        "失業率",
        "中性"
      ],
      "summary": "失業率 將於 2026/06/05 20:30 公布，市場關注前值 8.2、預期 —。",
      "event": "美國 失業率，前值 8.2、預期 —、實際 8.1。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-manufacturing-payrolls-may-20260605",
      "type": "macro",
      "title": "非農就業人數",
      "eventName": "非農就業人數",
      "originalEventName": "Manufacturing Payrolls (May)",
      "sourcePublishTime": "2026/06/05 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/05 20:30",
      "previous": "177",
      "forecast": "2",
      "actual": "7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "高",
      "timestamp": "2026/06/05 20:30",
      "tags": [
        "美國",
        "非農就業人數",
        "偏多"
      ],
      "summary": "非農就業人數 將於 2026/06/05 20:30 公布，市場關注前值 —、預期 2。",
      "event": "美國 非農就業人數，前值 —、預期 2、實際 7。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-weekly-20260609",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change Weekly",
      "sourcePublishTime": "2026/06/09 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/09 20:15",
      "previous": "30.5",
      "forecast": "—",
      "actual": "29",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/09 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/06/09 20:15 公布，市場關注前值 30.5、預期 —。",
      "event": "美國 ADP 就業人數，前值 30.5、預期 —、實際 29。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-core-inflation-rate-mom-may-20260610",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate MoM (May)",
      "sourcePublishTime": "2026/06/10 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/10 20:30",
      "previous": "0.4",
      "forecast": "0.3",
      "actual": "0.2",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/10 20:30",
      "tags": [
        "美國",
        "核心通膨率",
        "偏多"
      ],
      "summary": "核心通膨率 將於 2026/06/10 20:30 公布，市場關注前值 0.4、預期 0.3。",
      "event": "美國 核心通膨率，前值 0.4、預期 0.3、實際 0.2。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-may-20260610",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM (May)",
      "sourcePublishTime": "2026/06/10 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/10 20:30",
      "previous": "0.6",
      "forecast": "0.5",
      "actual": "0.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/10 20:30",
      "tags": [
        "美國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/10 20:30 公布，市場關注前值 0.6、預期 0.5。",
      "event": "美國 通膨率，前值 0.6、預期 0.5、實際 0.5。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-may-20260610",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "CPI (May)",
      "sourcePublishTime": "2026/06/10 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/10 20:30",
      "previous": "333.02",
      "forecast": "335.11",
      "actual": "335.12",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/10 20:30",
      "tags": [
        "美國",
        "消費者物價指數",
        "偏空"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/10 20:30 公布，市場關注前值 333.02、預期 335.11。",
      "event": "美國 消費者物價指數 CPI，前值 333.02、預期 335.11、實際 335.12。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "http://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-5-20260610",
      "type": "macro",
      "title": "核心 CPI",
      "eventName": "核心 CPI",
      "originalEventName": "核心消費價格指數(CPI) (月環比) (5月)",
      "sourcePublishTime": "2026/06/10 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/10 20:30",
      "previous": "0.4%",
      "forecast": "0.3%",
      "actual": "0.2%",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/10 20:30",
      "tags": [
        "美國",
        "核心",
        "偏多"
      ],
      "summary": "核心 CPI 將於 2026/06/10 20:30 公布，市場關注前值 0.4%、預期 0.3%。",
      "event": "美國 核心 CPI，前值 0.4%、預期 0.3%、實際 0.2%。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/549987",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-ecb-interest-rate-decision-20260611",
      "type": "macro",
      "title": "ECB 利率決議",
      "eventName": "ECB 利率決議",
      "originalEventName": "ECB Interest Rate Decision",
      "sourcePublishTime": "2026/06/11 20:15 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/11 20:15",
      "previous": "2.15",
      "forecast": "2.4",
      "actual": "2.4",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/11 20:15",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB 利率決議 將於 2026/06/11 20:15 公布，市場關注前值 2.15、預期 2.4。",
      "event": "歐元區 ECB 利率決議，前值 2.15、預期 2.4、實際 2.4。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-initial-jobless-claims-jun-06-20260611",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Initial Jobless Claims (Jun/06)",
      "sourcePublishTime": "2026/06/11 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/11 20:30",
      "previous": "225",
      "forecast": "219",
      "actual": "229",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/11 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "偏多"
      ],
      "summary": "初領失業救濟金人數 將於 2026/06/11 20:30 公布，市場關注前值 225、預期 219。",
      "event": "美國 初領失業救濟金人數，前值 225、預期 219、實際 229。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.dol.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-ppi-mom-may-20260611",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "Core PPI MoM (May)",
      "sourcePublishTime": "2026/06/11 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/11 20:30",
      "previous": "0.7",
      "forecast": "0.5",
      "actual": "0.4",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/11 20:30",
      "tags": [
        "美國",
        "生產者物價指數",
        "偏空"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/06/11 20:30 公布，市場關注前值 0.7、預期 0.5。",
      "event": "美國 生產者物價指數 PPI，前值 0.7、預期 0.5、實際 0.4。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-gdp-3-month-avg-apr-20260612",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "GDP 3-Month Avg (Apr)",
      "sourcePublishTime": "2026/06/12 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/12 14:00",
      "previous": "0.6",
      "forecast": "0.7",
      "actual": "0.7",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/06/12 14:00",
      "tags": [
        "英國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/12 14:00 公布，市場關注前值 0.6、預期 0.7。",
      "event": "英國 GDP 經濟成長率，前值 0.6、預期 0.7、實際 0.7。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-harmonised-inflation-rate-mom-final-may-20260612",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Harmonised Inflation Rate MoM Final (May)",
      "sourcePublishTime": "2026/06/12 14:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/12 14:00",
      "previous": "0.5",
      "forecast": "-0.1",
      "actual": "-0.1",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/12 14:00",
      "tags": [
        "德國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/12 14:00 公布，市場關注前值 0.5、預期 -0.1。",
      "event": "德國 通膨率，前值 0.5、預期 -0.1、實際 -0.1。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.destatis.de",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-5-20260612",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "德國居民消費價格指數(CPI) (月環比) (5月)",
      "sourcePublishTime": "2026/06/12 14:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/12 14:00",
      "previous": "0.6%",
      "forecast": "-0.2%",
      "actual": "-0.2%",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/12 14:00",
      "tags": [
        "德國",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/12 14:00 公布，市場關注前值 0.6%、預期 -0.2%。",
      "event": "德國 消費者物價指數 CPI，前值 0.6%、預期 -0.2%、實際 -0.2%。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/550129",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-harmonised-inflation-rate-mom-final-may-20260612",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Harmonised Inflation Rate MoM Final (May)",
      "sourcePublishTime": "2026/06/12 14:45 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/12 14:45",
      "previous": "1.2",
      "forecast": "0.1",
      "actual": "0.1",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/12 14:45",
      "tags": [
        "法國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/12 14:45 公布，市場關注前值 1.2、預期 0.1。",
      "event": "法國 通膨率，前值 1.2、預期 0.1、實際 0.1。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.insee.fr",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-niesr-monthly-gdp-tracker-may-20260612",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "NIESR Monthly GDP Tracker (May)",
      "sourcePublishTime": "2026/06/12 18:50 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/12 18:50",
      "previous": "0.7",
      "forecast": "—",
      "actual": "0.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/06/12 18:50",
      "tags": [
        "英國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/12 18:50 公布，市場關注前值 0.7、預期 —。",
      "event": "英國 GDP 經濟成長率，前值 0.7、預期 —、實際 0.5。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://tw.tradingview.com/economic-calendar/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-niesr-gdp-20260612",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "NIESR每月GDP追蹤",
      "sourcePublishTime": "2026/06/12 19:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/12 19:00",
      "previous": "0.8",
      "forecast": "0.7",
      "actual": "0.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏空",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/06/12 19:00",
      "tags": [
        "英國",
        "GDP",
        "偏空"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/12 19:00 公布，市場關注前值 0.8、預期 0.7。",
      "event": "英國 GDP 經濟成長率，前值 0.8、預期 0.7、實際 0.5。",
      "ai": "目前 AI 判斷為偏空觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "鉅亨網全球經濟指標",
      "sourceUrl": "https://www.cnyes.com/economy/indicator",
      "sourceList": [
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ecb-survey-of-monetary-analysts-20260615",
      "type": "macro",
      "title": "ECB Survey of Monetary Analysts",
      "eventName": "ECB Survey of Monetary Analysts",
      "originalEventName": "ECB Survey of Monetary Analysts",
      "sourcePublishTime": "2026/06/15 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/15 16:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "等待公布",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/15 16:00",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB Survey of Monetary Analysts 將於 2026/06/15 16:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 ECB Survey of Monetary Analysts，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-weekly-20260616",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change Weekly",
      "sourcePublishTime": "2026/06/16 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/16 20:15",
      "previous": "29",
      "forecast": "—",
      "actual": "25.5",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/16 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/06/16 20:15 公布，市場關注前值 29、預期 —。",
      "event": "美國 ADP 就業人數，前值 29、預期 —、實際 25.5。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-ppi-core-output-mom-may-20260617",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI Core Output MoM (May)",
      "sourcePublishTime": "2026/06/17 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/17 14:00",
      "previous": "0.9",
      "forecast": "0.4",
      "actual": "0.8",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/17 14:00",
      "tags": [
        "英國",
        "生產者物價指數",
        "偏多"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/06/17 14:00 公布，市場關注前值 0.9、預期 0.4。",
      "event": "英國 生產者物價指數 PPI，前值 0.9、預期 0.4、實際 0.8。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-may-20260617",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM (May)",
      "sourcePublishTime": "2026/06/17 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/17 14:00",
      "previous": "0.7",
      "forecast": "0.4",
      "actual": "0.2",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/17 14:00",
      "tags": [
        "英國",
        "通膨率",
        "偏多"
      ],
      "summary": "通膨率 將於 2026/06/17 14:00 公布，市場關注前值 0.7、預期 0.4。",
      "event": "英國 通膨率，前值 0.7、預期 0.4、實際 0.2。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-inflation-rate-yoy-may-20260617",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate YoY (May)",
      "sourcePublishTime": "2026/06/17 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/17 14:00",
      "previous": "2.5",
      "forecast": "2.7",
      "actual": "2.6",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/17 14:00",
      "tags": [
        "英國",
        "核心通膨率",
        "偏多"
      ],
      "summary": "核心通膨率 將於 2026/06/17 14:00 公布，市場關注前值 2.5、預期 2.7。",
      "event": "英國 核心通膨率，前值 2.5、預期 2.7、實際 2.6。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-5-20260617",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "居民消費價格指數(CPI) (同比) (5月)",
      "sourcePublishTime": "2026/06/17 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/17 14:00",
      "previous": "2.8%",
      "forecast": "3.0%",
      "actual": "2.8%",
      "status": "已公布",
      "statusLevel": "published",
      "direction": "偏多",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/17 14:00",
      "tags": [
        "英國",
        "消費者物價指數",
        "偏多"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/17 14:00 公布，市場關注前值 2.8%、預期 3.0%。",
      "event": "英國 消費者物價指數 CPI，前值 2.8%、預期 3.0%、實際 2.8%。",
      "ai": "目前 AI 判斷為偏多觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/550356",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-final-may-20260617",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM Final (May)",
      "sourcePublishTime": "2026/06/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/17 17:00",
      "previous": "1",
      "forecast": "0.1",
      "actual": null,
      "status": "倒數 10 小時",
      "statusLevel": "soon",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/17 17:00",
      "tags": [
        "歐元區",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/17 17:00 公布，市場關注前值 1、預期 0.1。",
      "event": "歐元區 通膨率，前值 1、預期 0.1、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-final-may-20260617",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "CPI Final (May)",
      "sourcePublishTime": "2026/06/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/17 17:00",
      "previous": "103.04",
      "forecast": "103.15",
      "actual": null,
      "status": "倒數 10 小時",
      "statusLevel": "soon",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/17 17:00",
      "tags": [
        "歐元區",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/17 17:00 公布，市場關注前值 103.04、預期 103.15。",
      "event": "歐元區 消費者物價指數 CPI，前值 103.04、預期 103.15、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-inflation-rate-yoy-final-may-20260617",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate YoY Final (May)",
      "sourcePublishTime": "2026/06/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/17 17:00",
      "previous": "2.2",
      "forecast": "2.5",
      "actual": null,
      "status": "倒數 10 小時",
      "statusLevel": "soon",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/17 17:00",
      "tags": [
        "歐元區",
        "核心通膨率",
        "中性"
      ],
      "summary": "核心通膨率 將於 2026/06/17 17:00 公布，市場關注前值 2.2、預期 2.5。",
      "event": "歐元區 核心通膨率，前值 2.2、預期 2.5、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-fed-interest-rate-decision-20260618",
      "type": "macro",
      "title": "FOMC 利率決議",
      "eventName": "FOMC 利率決議",
      "originalEventName": "Fed Interest Rate Decision",
      "sourcePublishTime": "2026/06/18 02:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/18 02:00",
      "previous": "3.75",
      "forecast": "3.75",
      "actual": null,
      "status": "倒數 19 小時",
      "statusLevel": "soon",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/18 02:00",
      "tags": [
        "美國",
        "FOMC",
        "中性"
      ],
      "summary": "FOMC 利率決議 將於 2026/06/18 02:00 公布，市場關注前值 3.75、預期 3.75。",
      "event": "美國 FOMC 利率決議，前值 3.75、預期 3.75、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "http://www.federalreserve.gov/",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-fomc-20260618",
      "type": "macro",
      "title": "FOMC 利率決議",
      "eventName": "FOMC 利率決議",
      "originalEventName": "美聯儲(FOMC)新聞發布會",
      "sourcePublishTime": "2026/06/18 02:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/18 02:30",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 小時",
      "statusLevel": "soon",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "高",
      "timestamp": "2026/06/18 02:30",
      "tags": [
        "美國",
        "FOMC",
        "中性"
      ],
      "summary": "FOMC 利率決議 將於 2026/06/18 02:30 公布，市場關注前值 —、預期 —。",
      "event": "美國 FOMC 利率決議，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/551455",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-hmrc-payrolls-change-may-20260618",
      "type": "macro",
      "title": "非農就業人數",
      "eventName": "非農就業人數",
      "originalEventName": "HMRC Payrolls Change (May)",
      "sourcePublishTime": "2026/06/18 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/18 14:00",
      "previous": "-100",
      "forecast": "—",
      "actual": null,
      "status": "倒數 1 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "高",
      "timestamp": "2026/06/18 14:00",
      "tags": [
        "英國",
        "非農就業人數",
        "中性"
      ],
      "summary": "非農就業人數 將於 2026/06/18 14:00 公布，市場關注前值 -100、預期 —。",
      "event": "英國 非農就業人數，前值 -100、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-unemployment-rate-apr-20260618",
      "type": "macro",
      "title": "失業率",
      "eventName": "失業率",
      "originalEventName": "Unemployment Rate (Apr)",
      "sourcePublishTime": "2026/06/18 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/18 14:00",
      "previous": "5",
      "forecast": "5",
      "actual": null,
      "status": "倒數 1 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/18 14:00",
      "tags": [
        "英國",
        "失業率",
        "中性"
      ],
      "summary": "失業率 將於 2026/06/18 14:00 公布，市場關注前值 5、預期 5。",
      "event": "英國 失業率，前值 5、預期 5、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-employment-change-apr-20260618",
      "type": "macro",
      "title": "就業人數變化",
      "eventName": "就業人數變化",
      "originalEventName": "Employment Change (Apr)",
      "sourcePublishTime": "2026/06/18 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/18 14:00",
      "previous": "148",
      "forecast": "75",
      "actual": null,
      "status": "倒數 1 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/18 14:00",
      "tags": [
        "英國",
        "就業人數變化",
        "中性"
      ],
      "summary": "就業人數變化 將於 2026/06/18 14:00 公布，市場關注前值 148、預期 75。",
      "event": "英國 就業人數變化，前值 148、預期 75、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-boe-interest-rate-decision-20260618",
      "type": "macro",
      "title": "英國央行利率決議",
      "eventName": "英國央行利率決議",
      "originalEventName": "BoE Interest Rate Decision",
      "sourcePublishTime": "2026/06/18 19:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/18 19:00",
      "previous": "3.75",
      "forecast": "3.75",
      "actual": null,
      "status": "倒數 1 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/18 19:00",
      "tags": [
        "英國",
        "英國央行利率決議",
        "中性"
      ],
      "summary": "英國央行利率決議 將於 2026/06/18 19:00 公布，市場關注前值 3.75、預期 3.75。",
      "event": "英國 英國央行利率決議，前值 3.75、預期 3.75、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.bankofengland.co.uk",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-continuing-jobless-claims-jun-06-20260618",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Continuing Jobless Claims (Jun/06)",
      "sourcePublishTime": "2026/06/18 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/18 20:30",
      "previous": "1795",
      "forecast": "1800",
      "actual": null,
      "status": "倒數 1 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/18 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "中性"
      ],
      "summary": "初領失業救濟金人數 將於 2026/06/18 20:30 公布，市場關注前值 1795、預期 1800。",
      "event": "美國 初領失業救濟金人數，前值 1795、預期 1800、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.dol.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ppi-yoy-may-20260619",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI YoY (May)",
      "sourcePublishTime": "2026/06/19 14:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/19 14:00",
      "previous": "1.7",
      "forecast": "2.5",
      "actual": null,
      "status": "倒數 2 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/19 14:00",
      "tags": [
        "德國",
        "生產者物價指數",
        "中性"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/06/19 14:00 公布，市場關注前值 1.7、預期 2.5。",
      "event": "德國 生產者物價指數 PPI，前值 1.7、預期 2.5、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.destatis.de",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:15 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/23 15:15",
      "previous": "44.9",
      "forecast": "45.1",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:15",
      "tags": [
        "法國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/23 15:15 公布，市場關注前值 44.9、預期 —。",
      "event": "法國 綜合 PMI，前值 44.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:15 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/23 15:15",
      "previous": "49.7",
      "forecast": "49.6",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:15",
      "tags": [
        "法國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/06/23 15:15 公布，市場關注前值 49.7、預期 —。",
      "event": "法國 製造業 PMI，前值 49.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:15 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/23 15:15",
      "previous": "44.3",
      "forecast": "44.0",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:15",
      "tags": [
        "法國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/23 15:15 公布，市場關注前值 44.3、預期 —。",
      "event": "法國 服務業 PMI，前值 44.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:30 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/23 15:30",
      "previous": "50.1",
      "forecast": "49.0",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:30",
      "tags": [
        "德國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/06/23 15:30 公布，市場關注前值 50.1、預期 —。",
      "event": "德國 製造業 PMI，前值 50.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:30 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/23 15:30",
      "previous": "48.8",
      "forecast": "47.9",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:30",
      "tags": [
        "德國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/23 15:30 公布，市場關注前值 48.8、預期 —。",
      "event": "德國 綜合 PMI，前值 48.8、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 15:30 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/23 15:30",
      "previous": "48.1",
      "forecast": "46.0",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 15:30",
      "tags": [
        "德國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/23 15:30 公布，市場關注前值 48.1、預期 —。",
      "event": "德國 服務業 PMI，前值 48.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/23 16:00",
      "previous": "48.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:00",
      "tags": [
        "歐元區",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/23 16:00 公布，市場關注前值 48.5、預期 —。",
      "event": "歐元區 綜合 PMI，前值 48.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/23 16:00",
      "previous": "51.6",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:00",
      "tags": [
        "歐元區",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/06/23 16:00 公布，市場關注前值 51.6、預期 —。",
      "event": "歐元區 製造業 PMI，前值 51.6、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/23 16:00",
      "previous": "47.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:00",
      "tags": [
        "歐元區",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/23 16:00 公布，市場關注前值 47.7、預期 —。",
      "event": "歐元區 服務業 PMI，前值 47.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/23 16:30",
      "previous": "49.3",
      "forecast": "51.9",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:30",
      "tags": [
        "英國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/23 16:30 公布，市場關注前值 49.3、預期 —。",
      "event": "英國 服務業 PMI，前值 49.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/23 16:30",
      "previous": "53.9",
      "forecast": "53.4",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:30",
      "tags": [
        "英國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/06/23 16:30 公布，市場關注前值 53.9、預期 —。",
      "event": "英國 製造業 PMI，前值 53.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/23 16:30",
      "previous": "49.7",
      "forecast": "52.1",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 16:30",
      "tags": [
        "英國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/23 16:30 公布，市場關注前值 49.7、預期 —。",
      "event": "英國 綜合 PMI，前值 49.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-adp-employment-change-weekly-20260623",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change Weekly",
      "sourcePublishTime": "2026/06/23 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 20:15",
      "previous": "25.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/06/23 20:15 公布，市場關注前值 25.5、預期 —。",
      "event": "美國 ADP 就業人數，前值 25.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 21:45",
      "previous": "55.1",
      "forecast": "54.5",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 21:45",
      "tags": [
        "美國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/06/23 21:45 公布，市場關注前值 55.1、預期 —。",
      "event": "美國 製造業 PMI，前值 55.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 21:45",
      "previous": "51.5",
      "forecast": "50.6",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 21:45",
      "tags": [
        "美國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/06/23 21:45 公布，市場關注前值 51.5、預期 —。",
      "event": "美國 綜合 PMI，前值 51.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-flash-jun-20260623",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Flash (Jun)",
      "sourcePublishTime": "2026/06/23 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 21:45",
      "previous": "50.7",
      "forecast": "50.4",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/23 21:45",
      "tags": [
        "美國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/06/23 21:45 公布，市場關注前值 50.7、預期 —。",
      "event": "美國 服務業 PMI，前值 50.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-richmond-fed-manufacturing-shipments-index-jun-20260623",
      "type": "macro",
      "title": "Richmond Fed Manufacturing Shipments Index (Jun)",
      "eventName": "Richmond Fed Manufacturing Shipments Index (Jun)",
      "originalEventName": "Richmond Fed Manufacturing Shipments Index (Jun)",
      "sourcePublishTime": "2026/06/23 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 22:00",
      "previous": "16",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/23 22:00",
      "tags": [
        "美國",
        "Richmond",
        "中性"
      ],
      "summary": "Richmond Fed Manufacturing Shipments Index (Jun) 將於 2026/06/23 22:00 公布，市場關注前值 16、預期 —。",
      "event": "美國 Richmond Fed Manufacturing Shipments Index (Jun)，前值 16、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.richmondfed.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-richmond-fed-manufacturing-index-jun-20260623",
      "type": "macro",
      "title": "Richmond Fed Manufacturing Index (Jun)",
      "eventName": "Richmond Fed Manufacturing Index (Jun)",
      "originalEventName": "Richmond Fed Manufacturing Index (Jun)",
      "sourcePublishTime": "2026/06/23 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/23 22:00",
      "previous": "13",
      "forecast": "—",
      "actual": null,
      "status": "倒數 6 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/23 22:00",
      "tags": [
        "美國",
        "Richmond",
        "中性"
      ],
      "summary": "Richmond Fed Manufacturing Index (Jun) 將於 2026/06/23 22:00 公布，市場關注前值 13、預期 —。",
      "event": "美國 Richmond Fed Manufacturing Index (Jun)，前值 13、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.richmondfed.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ecb-general-council-meeting-20260625",
      "type": "macro",
      "title": "ECB General Council Meeting",
      "eventName": "ECB General Council Meeting",
      "originalEventName": "ECB General Council Meeting",
      "sourcePublishTime": "2026/06/25 15:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/25 15:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/25 15:00",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB General Council Meeting 將於 2026/06/25 15:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 ECB General Council Meeting，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-continuing-jobless-claims-jun-13-20260625",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Continuing Jobless Claims (Jun/13)",
      "sourcePublishTime": "2026/06/25 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 20:30",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/25 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "中性"
      ],
      "summary": "初領失業救濟金人數 將於 2026/06/25 20:30 公布，市場關注前值 —、預期 —。",
      "event": "美國 初領失業救濟金人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.dol.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-pce-price-index-mom-may-20260625",
      "type": "macro",
      "title": "PCE 物價指數",
      "eventName": "PCE 物價指數",
      "originalEventName": "PCE Price Index MoM (May)",
      "sourcePublishTime": "2026/06/25 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 20:30",
      "previous": "0.4",
      "forecast": "4.5",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/25 20:30",
      "tags": [
        "美國",
        "PCE",
        "中性"
      ],
      "summary": "PCE 物價指數 將於 2026/06/25 20:30 公布，市場關注前值 0.4、預期 —。",
      "event": "美國 PCE 物價指數，前值 0.4、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "http://www.bea.gov/",
      "sourceList": [
        "TradingView Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-gdp-sales-qoq-final-q1-20260625",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "GDP Sales QoQ Final (Q1)",
      "sourcePublishTime": "2026/06/25 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 20:30",
      "previous": "0.3",
      "forecast": "1.5",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/06/25 20:30",
      "tags": [
        "美國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/25 20:30 公布，市場關注前值 0.3、預期 1.5。",
      "event": "美國 GDP 經濟成長率，前值 0.3、預期 1.5、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "http://www.bea.gov",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-core-pce-price-index-yoy-may-20260625",
      "type": "macro",
      "title": "核心 PCE 物價指數",
      "eventName": "核心 PCE 物價指數",
      "originalEventName": "Core PCE Price Index YoY (May)",
      "sourcePublishTime": "2026/06/25 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 20:30",
      "previous": "3.3",
      "forecast": "4.4",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/25 20:30",
      "tags": [
        "美國",
        "核心",
        "中性"
      ],
      "summary": "核心 PCE 物價指數 將於 2026/06/25 20:30 公布，市場關注前值 3.3、預期 —。",
      "event": "美國 核心 PCE 物價指數，前值 3.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar / 鉅亨網全球經濟指標",
      "sourceUrl": "http://www.bea.gov/",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar",
        "鉅亨網全球經濟指標"
      ]
    },
    {
      "id": "macro-kansas-fed-composite-index-jun-20260625",
      "type": "macro",
      "title": "Kansas Fed Composite Index (Jun)",
      "eventName": "Kansas Fed Composite Index (Jun)",
      "originalEventName": "Kansas Fed Composite Index (Jun)",
      "sourcePublishTime": "2026/06/25 23:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 23:00",
      "previous": "8",
      "forecast": "—",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/25 23:00",
      "tags": [
        "美國",
        "Kansas",
        "中性"
      ],
      "summary": "Kansas Fed Composite Index (Jun) 將於 2026/06/25 23:00 公布，市場關注前值 8、預期 —。",
      "event": "美國 Kansas Fed Composite Index (Jun)，前值 8、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://kansascityfed.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-kansas-fed-manufacturing-index-jun-20260625",
      "type": "macro",
      "title": "Kansas Fed Manufacturing Index (Jun)",
      "eventName": "Kansas Fed Manufacturing Index (Jun)",
      "originalEventName": "Kansas Fed Manufacturing Index (Jun)",
      "sourcePublishTime": "2026/06/25 23:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/25 23:00",
      "previous": "9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 8 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/25 23:00",
      "tags": [
        "美國",
        "Kansas",
        "中性"
      ],
      "summary": "Kansas Fed Manufacturing Index (Jun) 將於 2026/06/25 23:00 公布，市場關注前值 9、預期 —。",
      "event": "美國 Kansas Fed Manufacturing Index (Jun)，前值 9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://kansascityfed.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-unemployment-benefit-claims-may-20260626",
      "type": "macro",
      "title": "Unemployment Benefit Claims (May)",
      "eventName": "Unemployment Benefit Claims (May)",
      "originalEventName": "Unemployment Benefit Claims (May)",
      "sourcePublishTime": "2026/06/26 18:00 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/26 18:00",
      "previous": "-9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 9 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/26 18:00",
      "tags": [
        "法國",
        "Unemployment",
        "中性"
      ],
      "summary": "Unemployment Benefit Claims (May) 將於 2026/06/26 18:00 公布，市場關注前值 -9、預期 —。",
      "event": "法國 Unemployment Benefit Claims (May)，前值 -9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://dares.travail-emploi.gouv.fr",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ecb-forum-on-central-banking-20260629",
      "type": "macro",
      "title": "ECB Forum on Central Banking",
      "eventName": "ECB Forum on Central Banking",
      "originalEventName": "ECB Forum on Central Banking",
      "sourcePublishTime": "2026/06/29 08:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/29 08:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 12 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/29 08:00",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB Forum on Central Banking 將於 2026/06/29 08:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 ECB Forum on Central Banking，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-services-sentiment-jun-20260629",
      "type": "macro",
      "title": "Services Sentiment (Jun)",
      "eventName": "Services Sentiment (Jun)",
      "originalEventName": "Services Sentiment (Jun)",
      "sourcePublishTime": "2026/06/29 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/29 17:00",
      "previous": "2.2",
      "forecast": "—",
      "actual": null,
      "status": "倒數 12 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/29 17:00",
      "tags": [
        "歐元區",
        "Services",
        "中性"
      ],
      "summary": "Services Sentiment (Jun) 將於 2026/06/29 17:00 公布，市場關注前值 2.2、預期 —。",
      "event": "歐元區 Services Sentiment (Jun)，前值 2.2、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://ec.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-dallas-fed-manufacturing-index-jun-20260629",
      "type": "macro",
      "title": "Dallas Fed Manufacturing Index (Jun)",
      "eventName": "Dallas Fed Manufacturing Index (Jun)",
      "originalEventName": "Dallas Fed Manufacturing Index (Jun)",
      "sourcePublishTime": "2026/06/29 22:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/29 22:30",
      "previous": "0.4",
      "forecast": "—",
      "actual": null,
      "status": "倒數 12 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/29 22:30",
      "tags": [
        "美國",
        "Dallas",
        "中性"
      ],
      "summary": "Dallas Fed Manufacturing Index (Jun) 將於 2026/06/29 22:30 公布，市場關注前值 0.4、預期 —。",
      "event": "美國 Dallas Fed Manufacturing Index (Jun)，前值 0.4、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.dallasfed.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-brc-shop-price-inflation-jun-20260630",
      "type": "macro",
      "title": "BRC Shop Price Inflation (Jun)",
      "eventName": "BRC Shop Price Inflation (Jun)",
      "originalEventName": "BRC Shop Price Inflation (Jun)",
      "sourcePublishTime": "2026/06/30 07:01 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/30 07:01",
      "previous": "1.2",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/30 07:01",
      "tags": [
        "英國",
        "BRC",
        "中性"
      ],
      "summary": "BRC Shop Price Inflation (Jun) 將於 2026/06/30 07:01 公布，市場關注前值 1.2、預期 —。",
      "event": "英國 BRC Shop Price Inflation (Jun)，前值 1.2、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://brc.org.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ecb-forum-on-central-banking-20260630",
      "type": "macro",
      "title": "ECB Forum on Central Banking",
      "eventName": "ECB Forum on Central Banking",
      "originalEventName": "ECB Forum on Central Banking",
      "sourcePublishTime": "2026/06/30 08:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/06/30 08:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/30 08:00",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB Forum on Central Banking 將於 2026/06/30 08:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 ECB Forum on Central Banking，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-gdp-growth-rate-yoy-final-q1-20260630",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "GDP Growth Rate YoY Final (Q1)",
      "sourcePublishTime": "2026/06/30 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/06/30 14:00",
      "previous": "1",
      "forecast": "1.1",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "高",
      "timestamp": "2026/06/30 14:00",
      "tags": [
        "英國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/06/30 14:00 公布，市場關注前值 1、預期 1.1。",
      "event": "英國 GDP 經濟成長率，前值 1、預期 1.1、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-harmonised-inflation-rate-mom-prel-jun-20260630",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Harmonised Inflation Rate MoM Prel (Jun)",
      "sourcePublishTime": "2026/06/30 14:45 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/30 14:45",
      "previous": "0.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/30 14:45",
      "tags": [
        "法國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/30 14:45 公布，市場關注前值 0.1、預期 —。",
      "event": "法國 通膨率，前值 0.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.insee.fr",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ppi-yoy-may-20260630",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI YoY (May)",
      "sourcePublishTime": "2026/06/30 14:45 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/06/30 14:45",
      "previous": "2.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/30 14:45",
      "tags": [
        "法國",
        "生產者物價指數",
        "中性"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/06/30 14:45 公布，市場關注前值 2.1、預期 —。",
      "event": "法國 生產者物價指數 PPI，前值 2.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.insee.fr",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-unemployment-rate-jun-20260630",
      "type": "macro",
      "title": "失業率",
      "eventName": "失業率",
      "originalEventName": "Unemployment Rate (Jun)",
      "sourcePublishTime": "2026/06/30 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/30 15:55",
      "previous": "6.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/06/30 15:55",
      "tags": [
        "德國",
        "失業率",
        "中性"
      ],
      "summary": "失業率 將於 2026/06/30 15:55 公布，市場關注前值 6.3、預期 —。",
      "event": "德國 失業率，前值 6.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://statistik.arbeitsagentur.de",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-unemployment-change-jun-20260630",
      "type": "macro",
      "title": "就業人數變化",
      "eventName": "就業人數變化",
      "originalEventName": "Unemployment Change (Jun)",
      "sourcePublishTime": "2026/06/30 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/30 15:55",
      "previous": "-12",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/06/30 15:55",
      "tags": [
        "德國",
        "就業人數變化",
        "中性"
      ],
      "summary": "就業人數變化 將於 2026/06/30 15:55 公布，市場關注前值 -12、預期 —。",
      "event": "德國 就業人數變化，前值 -12、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://statistik.arbeitsagentur.de",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-brandenburg-cpi-yoy-jun-20260630",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "Brandenburg CPI YoY (Jun)",
      "sourcePublishTime": "2026/06/30 16:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/30 16:00",
      "previous": "2.8",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/30 16:00",
      "tags": [
        "德國",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/30 16:00 公布，市場關注前值 2.8、預期 —。",
      "event": "德國 消費者物價指數 CPI，前值 2.8、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.statistik-berlin-brandenburg.de/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-prel-jun-20260630",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM Prel (Jun)",
      "sourcePublishTime": "2026/06/30 20:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/30 20:00",
      "previous": "-0.2",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/06/30 20:00",
      "tags": [
        "德國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/06/30 20:00 公布，市場關注前值 -0.2、預期 —。",
      "event": "德國 通膨率，前值 -0.2、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.destatis.de",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-6-20260630",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "德國居民消費價格指數(CPI) (月環比) (6月)",
      "sourcePublishTime": "2026/06/30 20:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/06/30 20:00",
      "previous": "-0.2%",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/06/30 20:00",
      "tags": [
        "德國",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/06/30 20:00 公布，市場關注前值 -0.2%、預期 —。",
      "event": "德國 消費者物價指數 CPI，前值 -0.2%、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/551254",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-chicago-pmi-jun-20260630",
      "type": "macro",
      "title": "PMI 採購經理人指數",
      "eventName": "PMI 採購經理人指數",
      "originalEventName": "Chicago PMI (Jun)",
      "sourcePublishTime": "2026/06/30 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/06/30 21:45",
      "previous": "62.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 13 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/06/30 21:45",
      "tags": [
        "美國",
        "PMI",
        "中性"
      ],
      "summary": "PMI 採購經理人指數 將於 2026/06/30 21:45 公布，市場關注前值 62.7、預期 —。",
      "event": "美國 PMI 採購經理人指數，前值 62.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://chicago.ismworld.org/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ecb-forum-on-central-banking-20260701",
      "type": "macro",
      "title": "ECB Forum on Central Banking",
      "eventName": "ECB Forum on Central Banking",
      "originalEventName": "ECB Forum on Central Banking",
      "sourcePublishTime": "2026/07/01 08:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/01 08:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 08:00",
      "tags": [
        "歐元區",
        "ECB",
        "中性"
      ],
      "summary": "ECB Forum on Central Banking 將於 2026/07/01 08:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 ECB Forum on Central Banking，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ecb.europa.eu",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-final-jun-20260701",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Final (Jun)",
      "sourcePublishTime": "2026/07/01 15:50 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/07/01 15:50",
      "previous": "49.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 15:50",
      "tags": [
        "法國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/07/01 15:50 公布，市場關注前值 49.7、預期 —。",
      "event": "法國 製造業 PMI，前值 49.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-final-jun-20260701",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Final (Jun)",
      "sourcePublishTime": "2026/07/01 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/07/01 15:55",
      "previous": "50.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 15:55",
      "tags": [
        "德國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/07/01 15:55 公布，市場關注前值 50.1、預期 —。",
      "event": "德國 製造業 PMI，前值 50.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-final-jun-20260701",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Final (Jun)",
      "sourcePublishTime": "2026/07/01 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/01 16:00",
      "previous": "51.6",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 16:00",
      "tags": [
        "歐元區",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/07/01 16:00 公布，市場關注前值 51.6、預期 —。",
      "event": "歐元區 製造業 PMI，前值 51.6、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-final-jun-20260701",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Final (Jun)",
      "sourcePublishTime": "2026/07/01 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/07/01 16:30",
      "previous": "53.9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 16:30",
      "tags": [
        "英國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/07/01 16:30 公布，市場關注前值 53.9、預期 —。",
      "event": "英國 製造業 PMI，前值 53.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-flash-jun-20260701",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "CPI Flash (Jun)",
      "sourcePublishTime": "2026/07/01 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/01 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/07/01 17:00",
      "tags": [
        "歐元區",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/07/01 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 消費者物價指數 CPI，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-inflation-rate-yoy-flash-jun-20260701",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate YoY Flash (Jun)",
      "sourcePublishTime": "2026/07/01 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/01 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/01 17:00",
      "tags": [
        "歐元區",
        "核心通膨率",
        "中性"
      ],
      "summary": "核心通膨率 將於 2026/07/01 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 核心通膨率，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-yoy-flash-jun-20260701",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate YoY Flash (Jun)",
      "sourcePublishTime": "2026/07/01 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/01 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/01 17:00",
      "tags": [
        "歐元區",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/07/01 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 通膨率，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-jun-20260701",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change (Jun)",
      "sourcePublishTime": "2026/07/01 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 20:15",
      "previous": "122",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/07/01 20:15 公布，市場關注前值 122、預期 —。",
      "event": "美國 ADP 就業人數，前值 122、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-manufacturing-pmi-final-jun-20260701",
      "type": "macro",
      "title": "製造業 PMI",
      "eventName": "製造業 PMI",
      "originalEventName": "S&P Global Manufacturing PMI Final (Jun)",
      "sourcePublishTime": "2026/07/01 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 21:45",
      "previous": "55.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/01 21:45",
      "tags": [
        "美國",
        "製造業",
        "中性"
      ],
      "summary": "製造業 PMI 將於 2026/07/01 21:45 公布，市場關注前值 55.1、預期 —。",
      "event": "美國 製造業 PMI，前值 55.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-manufacturing-new-orders-jun-20260701",
      "type": "macro",
      "title": "ISM 製造業新訂單",
      "eventName": "ISM 製造業新訂單",
      "originalEventName": "ISM Manufacturing New Orders (Jun)",
      "sourcePublishTime": "2026/07/01 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 22:00",
      "previous": "56.8",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/01 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業新訂單 將於 2026/07/01 22:00 公布，市場關注前值 56.8、預期 —。",
      "event": "美國 ISM 製造業新訂單，前值 56.8、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-manufacturing-pmi-jun-20260701",
      "type": "macro",
      "title": "ISM 製造業指數",
      "eventName": "ISM 製造業指數",
      "originalEventName": "ISM Manufacturing PMI (Jun)",
      "sourcePublishTime": "2026/07/01 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 22:00",
      "previous": "54",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/01 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業指數 將於 2026/07/01 22:00 公布，市場關注前值 54、預期 —。",
      "event": "美國 ISM 製造業指數，前值 54、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-manufacturing-prices-jun-20260701",
      "type": "macro",
      "title": "ISM 製造業物價",
      "eventName": "ISM 製造業物價",
      "originalEventName": "ISM Manufacturing Prices (Jun)",
      "sourcePublishTime": "2026/07/01 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 22:00",
      "previous": "82.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/01 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業物價 將於 2026/07/01 22:00 公布，市場關注前值 82.1、預期 —。",
      "event": "美國 ISM 製造業物價，前值 82.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-manufacturing-employment-jun-20260701",
      "type": "macro",
      "title": "ISM 製造業就業",
      "eventName": "ISM 製造業就業",
      "originalEventName": "ISM Manufacturing Employment (Jun)",
      "sourcePublishTime": "2026/07/01 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/01 22:00",
      "previous": "48.6",
      "forecast": "—",
      "actual": null,
      "status": "倒數 14 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/01 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業就業 將於 2026/07/01 22:00 公布，市場關注前值 48.6、預期 —。",
      "event": "美國 ISM 製造業就業，前值 48.6、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-unemployment-rate-may-20260702",
      "type": "macro",
      "title": "失業率",
      "eventName": "失業率",
      "originalEventName": "Unemployment Rate (May)",
      "sourcePublishTime": "2026/07/02 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/02 17:00",
      "previous": "6.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 15 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/07/02 17:00",
      "tags": [
        "歐元區",
        "失業率",
        "中性"
      ],
      "summary": "失業率 將於 2026/07/02 17:00 公布，市場關注前值 6.3、預期 —。",
      "event": "歐元區 失業率，前值 6.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-continuing-jobless-claims-jun-20-20260702",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Continuing Jobless Claims (Jun/20)",
      "sourcePublishTime": "2026/07/02 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/02 20:30",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 15 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/07/02 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "中性"
      ],
      "summary": "初領失業救濟金人數 將於 2026/07/02 20:30 公布，市場關注前值 —、預期 —。",
      "event": "美國 初領失業救濟金人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.dol.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-u-6-unemployment-rate-jun-20260702",
      "type": "macro",
      "title": "失業率",
      "eventName": "失業率",
      "originalEventName": "U-6 Unemployment Rate (Jun)",
      "sourcePublishTime": "2026/07/02 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/02 20:30",
      "previous": "8.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 15 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響全球資金成本、美元走勢與風險資產評價。",
      "importance": "中高",
      "timestamp": "2026/07/02 20:30",
      "tags": [
        "美國",
        "失業率",
        "中性"
      ],
      "summary": "失業率 將於 2026/07/02 20:30 公布，市場關注前值 8.1、預期 —。",
      "event": "美國 失業率，前值 8.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響全球資金成本、美元走勢與風險資產評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-manufacturing-payrolls-jun-20260702",
      "type": "macro",
      "title": "非農就業人數",
      "eventName": "非農就業人數",
      "originalEventName": "Manufacturing Payrolls (Jun)",
      "sourcePublishTime": "2026/07/02 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/02 20:30",
      "previous": "7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 15 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "高",
      "timestamp": "2026/07/02 20:30",
      "tags": [
        "美國",
        "非農就業人數",
        "中性"
      ],
      "summary": "非農就業人數 將於 2026/07/02 20:30 公布，市場關注前值 7、預期 —。",
      "event": "美國 非農就業人數，前值 7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-jun-20260703",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 15:50 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/07/03 15:50",
      "previous": "44.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 15:50",
      "tags": [
        "法國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/03 15:50 公布，市場關注前值 44.3、預期 —。",
      "event": "法國 服務業 PMI，前值 44.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-jun-20260703",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 15:50 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/07/03 15:50",
      "previous": "44.9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 15:50",
      "tags": [
        "法國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/07/03 15:50 公布，市場關注前值 44.9、預期 —。",
      "event": "法國 綜合 PMI，前值 44.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-jun-20260703",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/07/03 15:55",
      "previous": "48.8",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 15:55",
      "tags": [
        "德國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/07/03 15:55 公布，市場關注前值 48.8、預期 —。",
      "event": "德國 綜合 PMI，前值 48.8、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-jun-20260703",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 15:55 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/07/03 15:55",
      "previous": "48.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 15:55",
      "tags": [
        "德國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/03 15:55 公布，市場關注前值 48.1、預期 —。",
      "event": "德國 服務業 PMI，前值 48.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-jun-20260703",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/03 16:00",
      "previous": "47.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 16:00",
      "tags": [
        "歐元區",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/03 16:00 公布，市場關注前值 47.7、預期 —。",
      "event": "歐元區 服務業 PMI，前值 47.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-jun-20260703",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 16:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/03 16:00",
      "previous": "48.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 16:00",
      "tags": [
        "歐元區",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/07/03 16:00 公布，市場關注前值 48.5、預期 —。",
      "event": "歐元區 綜合 PMI，前值 48.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-jun-20260703",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/07/03 16:30",
      "previous": "49.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 16:30",
      "tags": [
        "英國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/07/03 16:30 公布，市場關注前值 49.7、預期 —。",
      "event": "英國 綜合 PMI，前值 49.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-jun-20260703",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (Jun)",
      "sourcePublishTime": "2026/07/03 16:30 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/07/03 16:30",
      "previous": "49.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 16 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/03 16:30",
      "tags": [
        "英國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/03 16:30 公布，市場關注前值 49.3、預期 —。",
      "event": "英國 服務業 PMI，前值 49.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ppi-yoy-may-20260706",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI YoY (May)",
      "sourcePublishTime": "2026/07/06 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/06 17:00",
      "previous": "4.9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/06 17:00",
      "tags": [
        "歐元區",
        "生產者物價指數",
        "中性"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/07/06 17:00 公布，市場關注前值 4.9、預期 —。",
      "event": "歐元區 生產者物價指數 PPI，前值 4.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-services-pmi-final-jun-20260706",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "S&P Global Services PMI Final (Jun)",
      "sourcePublishTime": "2026/07/06 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 21:45",
      "previous": "50.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/06 21:45",
      "tags": [
        "美國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/06 21:45 公布，市場關注前值 50.7、預期 —。",
      "event": "美國 服務業 PMI，前值 50.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar / Investing.com Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar",
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-s-p-global-composite-pmi-final-jun-20260706",
      "type": "macro",
      "title": "綜合 PMI",
      "eventName": "綜合 PMI",
      "originalEventName": "S&P Global Composite PMI Final (Jun)",
      "sourcePublishTime": "2026/07/06 21:45 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 21:45",
      "previous": "51.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/06 21:45",
      "tags": [
        "美國",
        "綜合",
        "中性"
      ],
      "summary": "綜合 PMI 將於 2026/07/06 21:45 公布，市場關注前值 51.5、預期 —。",
      "event": "美國 綜合 PMI，前值 51.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.pmi.spglobal.com/public",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-employment-jun-20260706",
      "type": "macro",
      "title": "ISM 製造業就業",
      "eventName": "ISM 製造業就業",
      "originalEventName": "ISM Services Employment (Jun)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "47.9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業就業 將於 2026/07/06 22:00 公布，市場關注前值 47.9、預期 —。",
      "event": "美國 ISM 製造業就業，前值 47.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-new-orders-jun-20260706",
      "type": "macro",
      "title": "ISM 製造業新訂單",
      "eventName": "ISM 製造業新訂單",
      "originalEventName": "ISM Services New Orders (Jun)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "57.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業新訂單 將於 2026/07/06 22:00 公布，市場關注前值 57.3、預期 —。",
      "event": "美國 ISM 製造業新訂單，前值 57.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-business-activity-jun-20260706",
      "type": "macro",
      "title": "ISM 服務業指數",
      "eventName": "ISM 服務業指數",
      "originalEventName": "ISM Services Business Activity (Jun)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "57.7",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 服務業指數 將於 2026/07/06 22:00 公布，市場關注前值 57.7、預期 —。",
      "event": "美國 ISM 服務業指數，前值 57.7、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-pmi-jun-20260706",
      "type": "macro",
      "title": "服務業 PMI",
      "eventName": "服務業 PMI",
      "originalEventName": "ISM Services PMI (Jun)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "54.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "服務業",
        "中性"
      ],
      "summary": "服務業 PMI 將於 2026/07/06 22:00 公布，市場關注前值 54.5、預期 —。",
      "event": "美國 服務業 PMI，前值 54.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-services-prices-jun-20260706",
      "type": "macro",
      "title": "ISM 製造業物價",
      "eventName": "ISM 製造業物價",
      "originalEventName": "ISM Services Prices (Jun)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "71.3",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業物價 將於 2026/07/06 22:00 公布，市場關注前值 71.3、預期 —。",
      "event": "美國 ISM 製造業物價，前值 71.3、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.ismworld.org",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ism-pmi-6-20260706",
      "type": "macro",
      "title": "ISM 製造業指數",
      "eventName": "ISM 製造業指數",
      "originalEventName": "ISM非製造業PMI (6月)",
      "sourcePublishTime": "2026/07/06 22:00 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/06 22:00",
      "previous": "54.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 19 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "中高",
      "timestamp": "2026/07/06 22:00",
      "tags": [
        "美國",
        "ISM",
        "中性"
      ],
      "summary": "ISM 製造業指數 將於 2026/07/06 22:00 公布，市場關注前值 54.5、預期 —。",
      "event": "美國 ISM 製造業指數，前值 54.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "Investing.com Economic Calendar",
      "sourceUrl": "https://hk.investing.com/economic-calendar/551651",
      "sourceList": [
        "Investing.com Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-weekly-20260707",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change Weekly",
      "sourcePublishTime": "2026/07/07 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/07 20:15",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 20 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/07 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/07/07 20:15 公布，市場關注前值 —、預期 —。",
      "event": "美國 ADP 就業人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-initial-jobless-claims-jul-04-20260709",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Initial Jobless Claims (Jul/04)",
      "sourcePublishTime": "2026/07/09 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/09 20:30",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 22 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/07/09 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "中性"
      ],
      "summary": "初領失業救濟金人數 將於 2026/07/09 20:30 公布，市場關注前值 —、預期 —。",
      "event": "美國 初領失業救濟金人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.dol.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-harmonised-inflation-rate-mom-final-jun-20260710",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Harmonised Inflation Rate MoM Final (Jun)",
      "sourcePublishTime": "2026/07/10 14:00 Asia/Taipei",
      "country": "德國",
      "publishTime": "2026/07/10 14:00",
      "previous": "-0.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 23 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/10 14:00",
      "tags": [
        "德國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/07/10 14:00 公布，市場關注前值 -0.1、預期 —。",
      "event": "德國 通膨率，前值 -0.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.destatis.de",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-final-jun-20260710",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM Final (Jun)",
      "sourcePublishTime": "2026/07/10 14:45 Asia/Taipei",
      "country": "法國",
      "publishTime": "2026/07/10 14:45",
      "previous": "0.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 23 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/10 14:45",
      "tags": [
        "法國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/07/10 14:45 公布，市場關注前值 0.1、預期 —。",
      "event": "法國 通膨率，前值 0.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.insee.fr",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-adp-employment-change-weekly-20260714",
      "type": "macro",
      "title": "ADP 就業人數",
      "eventName": "ADP 就業人數",
      "originalEventName": "ADP Employment Change Weekly",
      "sourcePublishTime": "2026/07/14 20:15 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/14 20:15",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 27 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/14 20:15",
      "tags": [
        "美國",
        "ADP",
        "中性"
      ],
      "summary": "ADP 就業人數 將於 2026/07/14 20:15 公布，市場關注前值 —、預期 —。",
      "event": "美國 ADP 就業人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://adpemploymentreport.com/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-inflation-rate-yoy-jun-20260714",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate YoY (Jun)",
      "sourcePublishTime": "2026/07/14 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/14 20:30",
      "previous": "2.9",
      "forecast": "—",
      "actual": null,
      "status": "倒數 27 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/14 20:30",
      "tags": [
        "美國",
        "核心通膨率",
        "中性"
      ],
      "summary": "核心通膨率 將於 2026/07/14 20:30 公布，市場關注前值 2.9、預期 —。",
      "event": "美國 核心通膨率，前值 2.9、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-jun-20260714",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "CPI (Jun)",
      "sourcePublishTime": "2026/07/14 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/14 20:30",
      "previous": "335.12",
      "forecast": "—",
      "actual": null,
      "status": "倒數 27 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/07/14 20:30",
      "tags": [
        "美國",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/07/14 20:30 公布，市場關注前值 335.12、預期 —。",
      "event": "美國 消費者物價指數 CPI，前值 335.12、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-yoy-jun-20260714",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate YoY (Jun)",
      "sourcePublishTime": "2026/07/14 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/14 20:30",
      "previous": "4.2",
      "forecast": "—",
      "actual": null,
      "status": "倒數 27 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/14 20:30",
      "tags": [
        "美國",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/07/14 20:30 公布，市場關注前值 4.2、預期 —。",
      "event": "美國 通膨率，前值 4.2、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.bls.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-ppi-mom-jun-20260715",
      "type": "macro",
      "title": "生產者物價指數 PPI",
      "eventName": "生產者物價指數 PPI",
      "originalEventName": "PPI MoM (Jun)",
      "sourcePublishTime": "2026/07/15 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/15 20:30",
      "previous": "1.1",
      "forecast": "—",
      "actual": null,
      "status": "倒數 28 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響市場風險偏好與資金輪動。",
      "importance": "中高",
      "timestamp": "2026/07/15 20:30",
      "tags": [
        "美國",
        "生產者物價指數",
        "中性"
      ],
      "summary": "生產者物價指數 PPI 將於 2026/07/15 20:30 公布，市場關注前值 1.1、預期 —。",
      "event": "美國 生產者物價指數 PPI，前值 1.1、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響市場風險偏好與資金輪動。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.bls.gov",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-gdp-yoy-may-20260716",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "GDP YoY (May)",
      "sourcePublishTime": "2026/07/16 14:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/07/16 14:00",
      "previous": "1.2",
      "forecast": "—",
      "actual": null,
      "status": "倒數 29 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/07/16 14:00",
      "tags": [
        "英國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/07/16 14:00 公布，市場關注前值 1.2、預期 —。",
      "event": "英國 GDP 經濟成長率，前值 1.2、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "http://www.ons.gov.uk/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-niesr-monthly-gdp-tracker-jun-20260716",
      "type": "macro",
      "title": "GDP 經濟成長率",
      "eventName": "GDP 經濟成長率",
      "originalEventName": "NIESR Monthly GDP Tracker (Jun)",
      "sourcePublishTime": "2026/07/16 19:00 Asia/Taipei",
      "country": "英國",
      "publishTime": "2026/07/16 19:00",
      "previous": "0.5",
      "forecast": "—",
      "actual": null,
      "status": "倒數 29 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響景氣循環、原物料、工業與科技需求預期。",
      "importance": "高",
      "timestamp": "2026/07/16 19:00",
      "tags": [
        "英國",
        "GDP",
        "中性"
      ],
      "summary": "GDP 經濟成長率 將於 2026/07/16 19:00 公布，市場關注前值 0.5、預期 —。",
      "event": "英國 GDP 經濟成長率，前值 0.5、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響景氣循環、原物料、工業與科技需求預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://tw.tradingview.com/economic-calendar/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-initial-jobless-claims-jul-11-20260716",
      "type": "macro",
      "title": "初領失業救濟金人數",
      "eventName": "初領失業救濟金人數",
      "originalEventName": "Initial Jobless Claims (Jul/11)",
      "sourcePublishTime": "2026/07/16 20:30 Asia/Taipei",
      "country": "美國",
      "publishTime": "2026/07/16 20:30",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 29 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "importance": "中高",
      "timestamp": "2026/07/16 20:30",
      "tags": [
        "美國",
        "初領失業救濟金人數",
        "中性"
      ],
      "summary": "初領失業救濟金人數 將於 2026/07/16 20:30 公布，市場關注前值 —、預期 —。",
      "event": "美國 初領失業救濟金人數，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響就業強弱、薪資通膨與聯準會政策預期。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://www.dol.gov/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-core-inflation-rate-yoy-final-jun-20260717",
      "type": "macro",
      "title": "核心通膨率",
      "eventName": "核心通膨率",
      "originalEventName": "Core Inflation Rate YoY Final (Jun)",
      "sourcePublishTime": "2026/07/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/17 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 30 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/17 17:00",
      "tags": [
        "歐元區",
        "核心通膨率",
        "中性"
      ],
      "summary": "核心通膨率 將於 2026/07/17 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 核心通膨率，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-inflation-rate-mom-final-jun-20260717",
      "type": "macro",
      "title": "通膨率",
      "eventName": "通膨率",
      "originalEventName": "Inflation Rate MoM Final (Jun)",
      "sourcePublishTime": "2026/07/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/17 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 30 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "中高",
      "timestamp": "2026/07/17 17:00",
      "tags": [
        "歐元區",
        "通膨率",
        "中性"
      ],
      "summary": "通膨率 將於 2026/07/17 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 通膨率，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    },
    {
      "id": "macro-cpi-final-jun-20260717",
      "type": "macro",
      "title": "消費者物價指數 CPI",
      "eventName": "消費者物價指數 CPI",
      "originalEventName": "CPI Final (Jun)",
      "sourcePublishTime": "2026/07/17 17:00 Asia/Taipei",
      "country": "歐元區",
      "publishTime": "2026/07/17 17:00",
      "previous": "—",
      "forecast": "—",
      "actual": null,
      "status": "倒數 30 天",
      "statusLevel": "upcoming",
      "direction": "中性",
      "impact": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "importance": "高",
      "timestamp": "2026/07/17 17:00",
      "tags": [
        "歐元區",
        "消費者物價指數",
        "中性"
      ],
      "summary": "消費者物價指數 CPI 將於 2026/07/17 17:00 公布，市場關注前值 —、預期 —。",
      "event": "歐元區 消費者物價指數 CPI，前值 —、預期 —、實際 尚未公布。",
      "ai": "目前 AI 判斷為中性觀察；若實際值與預期差距擴大，台股科技、金融與原物料族群可能出現資金重估。",
      "impactDetail": "影響美債殖利率、降息預期、科技股與金融股評價。",
      "risk": "總經數據公布前後波動容易放大，需留意市場預期差與政策口徑變化。",
      "source": "TradingView Economic Calendar",
      "sourceUrl": "https://ec.europa.eu/eurostat/",
      "sourceList": [
        "TradingView Economic Calendar"
      ]
    }
  ],
  "impactGrid": [
    [
      "通膨數據 (CPI/PCE)",
      "負面影響",
      "負面影響",
      "中性",
      "中性",
      "正面影響",
      "正面影響"
    ],
    [
      "就業數據",
      "中性",
      "中性",
      "中性",
      "負面影響",
      "正面影響",
      "中性"
    ],
    [
      "利率決議 (FOMC)",
      "負面影響",
      "負面影響",
      "中性",
      "中性",
      "正面影響",
      "正面影響"
    ],
    [
      "GDP 經濟成長",
      "中性",
      "正面影響",
      "中性",
      "正面影響",
      "正面影響",
      "中性"
    ],
    [
      "ISM 製造業指數",
      "中性",
      "正面影響",
      "中性",
      "正面影響",
      "正面影響",
      "中性"
    ]
  ],
  "odds": [
    [
      "2026/07/06",
      "利率決議",
      "維持不變 62.3%",
      "降息 37.7%"
    ],
    [
      "2026/08/17",
      "利率決議",
      "維持不變 41.8%",
      "降息 58.2%"
    ],
    [
      "2026/10/05",
      "利率決議",
      "維持不變 22.5%",
      "降息 77.5%"
    ]
  ],
  "sourceCards": [
    [
      "追蹤範圍",
      "追蹤近 2 個月美國與歐洲重大總經事件：CPI、PCE、非農、失業率、PMI/ISM、GDP、FOMC/ECB。"
    ],
    [
      "分析邏輯",
      "比對前值、預期值與實際值，判斷數據偏多、偏空或中性。"
    ],
    [
      "更新頻率",
      "GitHub Actions 每 30 分鐘檢查公布時間，事件到期後自動補抓實際值。"
    ],
    [
      "影響評估",
      "依數據類型推估對科技股、金融股、傳產與匯率敏感族群的影響。"
    ]
  ],
  "sourceStatus": {
    "tradingView": "https://tw.tradingview.com/economic-calendar/",
    "investing": "https://hk.investing.com/economic-calendar",
    "cnyes": "https://www.cnyes.com/economy/indicator",
    "macroMicro": "https://www.macromicro.me/calendar#macro",
    "filter": "美國/歐洲重大數據：CPI/PCE/PPI、GDP、就業/失業、PMI/ISM、FOMC/ECB 利率；排除談話、債券拍賣、假期與低關聯事件。",
    "lookbackDays": 14,
    "horizonDays": 62,
    "calendarWindow": "2026-06-03~2026-08-18",
    "automation": "GitHub Actions macro-due-check every 30 minutes; fetch actual values after publish time.",
    "fetchedEvents": 148
  }
};
